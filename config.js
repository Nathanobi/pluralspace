...
document.getElementById('btn-export-data').addEventListener('click', async () => {
  const data = {
    ...
    proxys,
    profils,
    // images exclues par défaut car trop lourdes — option séparée
  };
...
document.getElementById('import-file-input').addEventListener('change', async function() {
  ...
  try {
    ...
    openConfirm('Importer ces données ? Les données actuelles seront FUSIONNÉES (pas remplacées).', async () => {
      ...
      toast(`Import terminé : +${added.prenoms} prénoms, +${added.tags} tags, +${added.proxys} proxys, +${added.profils} profils.`, 'success');
      renderPrenoms(); renderTagsPage(); renderTagsPrenomView(); renderProxys(); renderProfils();
      renderTagFilters(); renderImgTagFilters(); renderNoProxyBanner(); updateStats(); renderConfigStats();
    });
  } catch(e) {
...
document.getElementById('btn-pk-import').addEventListener('click', async () => {
  ...
  try {
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      // ── Prénom ──
      let prenom = prenoms.find(p => p.name.toLowerCase() === memberName.toLowerCase());
      if (!prenom) {
      ...
      // Associer les tags des groupes PK
      const memberTagIds = pkMemberIdToTagIds.get(m.id) || [];
      if (memberTagIds.length > 0) {
        let changed = false;
        for (const tid of memberTagIds) {
          if (!(prenom.tags||[]).includes(tid)) {
            prenom.tags = [...(prenom.tags||[]), tid];
            changed = true;
          }
        }
        if (changed) {
          await dbPut('prenoms', prenom);
          appendLog(`&nbsp;&nbsp;<span style="color:var(--text3);">✦ ${memberTagIds.length} tag(s) associé(s)</span>`);
        }
      }
      ...
      // ── Avatar ──
      if (m.avatar_url) {
        const existingImg = images.find(i2 => i2.prenomId === prenom.id);
        if (!existingImg) {
