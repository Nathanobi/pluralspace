...
(function() {
  ...
  function setCollapsed(collapsed) {
    ...
    btn.title = collapsed ? 'Agrandir la navigation' : 'Réduire la navigation';
    localStorage.setItem(STORAGE_KEY, collapsed ? '1' : '0');
  }
  btn.addEventListener('click', () => setCollapsed(!nav.classList.contains('nav-collapsed')));
  // Restaurer l'état sauvegardé
  if (localStorage.getItem(STORAGE_KEY) === '1') setCollapsed(true);
  // Sur mobile : collapsé par défaut si écran petit
  if (window.innerWidth <= 768 && localStorage.getItem(STORAGE_KEY) === null) setCollapsed(true);
})();
