...
async function saveTagEdit() {
  ...
  renderTagsPage(); renderTagFilters(); renderImgTagFilters(); renderPrenoms(); renderTagsPrenomView();
}
  ...
  container.querySelectorAll('[data-tag-del]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      openConfirm((n>0?`Supprimer le tag "${tag.name}" ? ${n} prénom${n>1?'s':''} perdront ce tag.`:`Supprimer le tag "${tag.name}" ?`), async () => {
        ...
        renderTagsPage(); renderTagFilters(); renderImgTagFilters(); renderPrenoms(); updateStats();
...
function renderTagsPrenomView() {
  ...
  // Filtres tags
  filterArea.innerHTML = tags.map(t => {
    const c   = getTagColor(t.color);
    const sel = tagsActiveFilter===t.id;
    return `<button class="tag-filter-pill${sel?' active-pill':''}" data-tag-filter="${t.id}"
      style="${sel?`background:${c.bg};border-color:${c.border};color:${c.text};`:''}">${esc(t.name)}</button>`;
  }).join('') +
  `<button class="tag-filter-pill${tagsActiveFilter==='untagged'?' active-pill':''}" data-tag-filter="untagged">Sans tag</button>`;

  filterArea.querySelectorAll('[data-tag-filter]').forEach(btn => {
    btn.addEventListener('click', () => {
      const v = btn.dataset.tagFilter;
      tagsActiveFilter = tagsActiveFilter===v ? null : v;
      renderTagsPrenomView();
    });
  });

  const sortedTags = tags.slice().sort((a,b) => a.name.localeCompare(b.name,'fr'));
  let html = '';
  const toShow = tagsActiveFilter===null
    ? sortedTags
    : (tagsActiveFilter==='untagged' ? [] : [tags.find(t=>t.id===tagsActiveFilter)].filter(Boolean));

  toShow.forEach(tag => {
    const c    = getTagColor(tag.color);
    const list = prenoms.filter(p => (p.tags||[]).includes(tag.id)).sort((a,b)=>a.name.localeCompare(b.name,'fr'));
    if (list.length===0) return;
    html += tagSectionHtml(tag.name, c.text, list);
  });
  if (tagsActiveFilter===null || tagsActiveFilter==='untagged') {
    const untagged = prenoms.filter(p => !p.tags||p.tags.length===0).sort((a,b)=>a.name.localeCompare(b.name,'fr'));
    if (untagged.length>0) html += tagSectionHtml('Sans tag', 'var(--text3)', untagged);
  }
